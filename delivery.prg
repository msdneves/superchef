...
static function dados(parametro)
       ...
       if parametro == 1
          titulo := 'Incluir'
       elseif parametro == 2
          id     := val(valor_coluna('grid_mesas','form_delivery',1))
          titulo := 'Alterar'
          dbselectarea('delivery')
          ...
          delivery->(dbseek(id))
          if found()
             x_nome := delivery->nome
             delivery->(ordsetfocus('nome'))
          else
...
static function excluir()
       ...
       local id := val(valor_coluna('grid_mesas','form_delivery',1))
       ...
       if .not. found()
          ...
          if msgyesno('Nome : '+alltrim(delivery->nome),'Excluir')
             if lock_reg()
                delivery->(dbdelete())
                delivery->(dbunlock())
                delivery->(dbgotop())
...
static function relacao()
       ...
       while .not. eof()
             ...
             @ linha,030 PRINT strzero(delivery->codigo,4) FONT 'courier new' SIZE 010
             @ linha,045 PRINT delivery->nome FONT 'courier new' SIZE 010
...
static function cabecalho(p_pagina)
       ...
       @ 010,070 PRINT 'RELAÇÃO DE DELIVERY' FONT 'courier new' SIZE 018 BOLD
...
static function gravar(parametro)
       ...
       if parametro == 1
          while .T.
                ...
                conta->(dbgotop())
                if lock_reg()
                   codigo := conta->c_delivery
                   replace c_delivery with c_delivery + 1
                   conta->(dbcommit())
                   conta->(dbunlock())
                   exit
                else
          ...
          dbselectarea('delivery')
          delivery->(dbappend())
          delivery->codigo := codigo
          delivery->nome   := form_dados.tbox_001.value
          delivery->(dbcommit())
          delivery->(dbgotop())
          form_dados.release
          atualizar()
...
static function pesquisar()
       ...
       local cPesq        := alltrim(form_delivery.tbox_pesquisa.value)
       local lGridFreeze  := .T.
       local nTamNomePesq := len(cPesq)

       dbselectarea('delivery')
       delivery->(ordsetfocus('nome'))
       delivery->(dbseek(cPesq))

       if lGridFreeze
          form_delivery.grid_mesas.disableupdate
       endif

       delete item all from grid_mesas of form_delivery

       while .not. eof()
             if substr(field->nome,1,nTamNomePesq) == cPesq
                add item {str(delivery->codigo),alltrim(delivery->nome)} to grid_mesas of form_delivery
       ...
       if lGridFreeze
          form_delivery.grid_mesas.enableupdate
       endif
