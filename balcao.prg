...
static function dados(parametro)
       ...
       if parametro == 1
          titulo := 'Incluir'
       elseif parametro == 2
          id     := val(valor_coluna('grid_balcao','form_balcao',1))
          titulo := 'Alterar'
          dbselectarea('balcao')
...
static function excluir()
       ...
       local id := val(valor_coluna('grid_balcao','form_balcao',1))
       ...
       if .not. found()
          ...
          if msgyesno('Nome : '+alltrim(balcao->nome),'Excluir')
             if lock_reg()
                balcao->(dbdelete())
                balcao->(dbunlock())
                balcao->(dbgotop())
...
static function relacao()
       ...
       while .not. eof()
             ...
             @ linha,030 PRINT strzero(balcao->codigo,4) FONT 'courier new' SIZE 010
             @ linha,045 PRINT balcao->nome FONT 'courier new' SIZE 010
...
static function cabecalho(p_pagina)
       ...
       @ 010,070 PRINT 'RELAÇÃO DE BALCÃO' FONT 'courier new' SIZE 018 BOLD
...
static function gravar(parametro)
       ...
       if parametro == 1
          while .T.
                ...
                conta->(dbgotop())
                if lock_reg()
                   codigo := conta->c_balcao
                   replace c_balcao with c_balcao + 1
                   conta->(dbcommit())
                   conta->(dbunlock())
                   exit
                else
          ...
          dbselectarea('balcao')
          balcao->(dbappend())
          balcao->codigo := codigo
          balcao->nome   := form_dados.tbox_001.value
          balcao->(dbcommit())
          balcao->(dbgotop())
          form_dados.release
          atualizar()
...
static function pesquisar()
       ...
       local cPesq        := alltrim(form_balcao.tbox_pesquisa.value)
       local lGridFreeze  := .T.
       ...
       if lGridFreeze
          form_balcao.grid_balcao.disableupdate
       endif

       delete item all from grid_balcao of form_balcao

       while .not. eof()
             if substr(field->nome,1,nTamNomePesq) == cPesq
                add item {str(balcao->codigo),alltrim(balcao->nome)} to grid_balcao of form_balcao
       ...
       if lGridFreeze
          form_balcao.grid_balcao.enableupdate
       endif
